...
# Stratified resampling function (unchanged)
...
# Particle filter for DLM with latent regime switching (unchanged)
def particle_filter_dlm(y, time_index, num_particles=2000, lag=5):
    ...
    return filtered_level, beta_mean, beta_lower, beta_upper, particles_level, weights

# Main runner function integrating train/test split from utils.py
def run_dlm_numpyro(df, target_col, train_start="2000-01-01", train_end="2019-12-31", test_start="2020-01-01", test_end=None, num_particles=5000, lag=5):
    if test_end is None:
        test_end = df.index.max().strftime("%Y-%m-%d")

    # Use the utils.py split function
    ...
    time_index = y_train.dropna().index

    smoothed, beta_mean, beta_lower, beta_upper, particles, weights = particle_filter_dlm(
        y_train_np, time_index, num_particles=num_particles, lag=lag
    )

    residuals = y_train_np - smoothed
    burn_in = len(y_train_np) // 10
    ...
    plt.subplot(2, 1, 1)
    plt.plot(time_index.to_numpy(), y_train_np, label="Observed", color="blue", linewidth=2)
    plt.plot(time_index.to_numpy(), beta_mean, label="SMC Estimate", linestyle="--", color="red", linewidth=2)
    plt.fill_between(time_index.to_numpy(), beta_lower, beta_upper, alpha=0.3, color="red", label="90% CI")
    ...
    plt.subplot(2, 1, 2)
    plt.plot(time_index.to_numpy(), residuals, 'g-', alpha=0.7, label="Residuals")
    ...
    print(f"NumPyro SMC DLM RMSE on training: {rmse:.4f}")
